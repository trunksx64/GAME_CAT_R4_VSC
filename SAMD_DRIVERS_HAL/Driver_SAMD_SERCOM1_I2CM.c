...
void vDriver_SAMD_SERCOM1_I2CM_CONFIGURE(void) {
    ...
    GCLK_REGS->GCLK_CLKCTRL = (
            GCLK_CLKCTRL_ID_SERCOM1_CORE | // Generic Clock Selection ID
    ...
    while (SERCOM1_REGS->I2CM.SERCOM_SYNCBUSY & SERCOM_I2CM_SYNCBUSY_SWRST(pdSET)); // Wait Process Complete

    /* CFG ::: SERCOM1 (I2C) :: CTRLA --------------------------------- */
    SERCOM1_REGS->I2CM.SERCOM_CTRLA = (
            SERCOM_I2CM_CTRLA_LOWTOUTEN(pdCLR) | // SCL Low Time-Out
    ...
    SERCOM1_REGS->I2CM.SERCOM_CTRLB = (
            SERCOM_I2CM_CTRLB_CMD(0U) | // Command (No action)
    ...
    while (SERCOM1_REGS->I2CM.SERCOM_SYNCBUSY & SERCOM_I2CM_SYNCBUSY_ENABLE(pdSET)); // Wait Process Complete
...
void vDriver_SAMD_SERCOM1_I2C_SET_ACK(uint01_t ACK) {
    ...
    if (ACK == pdSET) SERCOM1_REGS->I2CM.SERCOM_CTRLB &= ~SERCOM_I2CM_CTRLB_ACKACT(pdSET);
    else SERCOM1_REGS->I2CM.SERCOM_CTRLB |= SERCOM_I2CM_CTRLB_ACKACT(pdSET);
