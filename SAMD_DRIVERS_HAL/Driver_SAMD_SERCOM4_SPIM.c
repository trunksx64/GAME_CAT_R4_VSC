...
void vDriver_SAMD_SERCOM4_SPIM_CONFIGURE(void) {
    ...
    GCLK_REGS->GCLK_CLKCTRL = (
            GCLK_CLKCTRL_ID_SERCOM4_CORE | // Generic Clock Selection ID
    ...
    while (SERCOM4_REGS->SPIM.SERCOM_SYNCBUSY & SERCOM_SPIM_SYNCBUSY_SWRST(pdSET)); // Wait Process Complete

    /* Configuration ::: SERCOM4 (SPI) :: CTRLA ----------------------------- */
    SERCOM4_REGS->SPIM.SERCOM_CTRLA = (
    ...
    SERCOM4_REGS->SPIM.SERCOM_CTRLB = (
    ...
    while (SERCOM4_REGS->SPIM.SERCOM_SYNCBUSY & SERCOM_SPIM_SYNCBUSY_CTRLB(pdSET)); // Wait Process Complete
    ...
    while (SERCOM4_REGS->SPIM.SERCOM_SYNCBUSY & SERCOM_SPIM_SYNCBUSY_ENABLE(pdSET)); // Wait Process Complete
